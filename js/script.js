...
const renderMovies = async (movie, page=1)=> {
  try {
    if (elSelect.value === "Movie") {
      let respone = await fetch(`https://www.omdbapi.com/?s=${movie}&apikey=6150da7e&page=${page}`);
      const data = await respone.json();
      renderMovie(data)
    } 
    if (elSelect.value === "series") {
      let respone = await fetch(`https://www.omdbapi.com/?t=${movie}&apikey=6150da7e&Season=${page}`)
      const data = await respone.json();
